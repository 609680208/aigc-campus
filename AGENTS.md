...
- **管理能力**：班级与学生管理、算力配额分配与大额审批、审计日志、模型接入配置
...
│   │   └── seed.js            # 种子数据（演示账号、班级）
│   └── src/
│       ├── auth/              # OPC SSO 注册/登录 + 个人资料 / 修改密码
│       ├── users/             # 用户 CRUD、批量导入、配额调整、班级管理
│       ├── models/            # AI 模型注册（多服务商）+ 启停（超管）
│       ├── works/             # 创作提交、作品列表、平台统计
│       ├── admin/             # 后台统计 / 审批 / 审计 / 用量分析
...
└── frontend/
    ...
    └── src/
        ...
        ├── stores/auth.ts     # 登录态、角色 getter（isAdmin/isSuper/isLeader/backendRole）
        ...
        └── views/
            ...
            ├── admin/Admin.vue            # 后台管理（老师/领导/超管三套导航，全部真实数据）
            └── settings/Settings.vue      # 系统设置（超管）
```

## 4. 角色权限体系

数据库角色枚举（`UserRole`）：`STUDENT` / `ADMIN` / `SUPER_ADMIN`；
管理员子角色（`AdminSubRole`）：`TEACHER`（老师）/ `LEADER`（校领导）。

| 端表现 | 数据库映射 | 可见后台导航 |
|---|---|---|
| 学生 · 普通 | `STUDENT` | 无后台入口 |
| 老师 · 管理员 | `ADMIN` + `TEACHER` | 创作历史 / 班级概览 / 算力配额 / 学生管理 |
| 领导 · 管理员 | `ADMIN` + `LEADER` | 全局看板 / 大额审批 / 审计日志 / 运营趋势 |
| 超级管理员 | `SUPER_ADMIN` | 权限管理 / 教师管理 / 学生管理 / 配额分配 / 算力账单 + 系统设置 |

- 权限判定：后端 `@Roles('ADMIN','SUPER_ADMIN')` 装饰器 + 全局 `RolesGuard`；前端路由 `meta.roles` + 守卫，页面内用 `auth.isAdmin / auth.isSuper / auth.isLeader` 控制显隐。
- 用户账号统一由 OPC 平台注册时同步创建（初始算力 100 点，角色 STUDENT）；教师与领导账号由超管在「权限管理」中分配。

## 5. 数据模型（schema.prisma 摘要）

- **User**：username（唯一）/ password（bcrypt）/ name / role / adminSubRole? / classId? / quotaBalance（默认 100）/ opcUserId?（唯一，OPC 用户 ID）/ ssoToken?（唯一，我方签发的永久 SSO token）/ apiKey?（genzhi API Key，SSO 带入）/ teamId?（OPC 团队）/ teamTaskId?（OPC 任务）
- **Class**：name（唯一）/ grade? / students[]
- **Model**：name + type（TEXT/TXT2IMG/IMG2IMG/TXT2VIDEO/IMG2VIDEO/AUDIO）唯一 / loc（CLOUD/LOCAL）/ cost / enabled / provider（String：OPENAI/DEEPSEEK/DASHSCOPE/CLAUDE/COMFYUI）/ apiKey? / baseUrl? / externalId?
- **Work**：userId / modelId? / type（WorkType，含 CANVAS 与 AUDIO）/ prompt / status（PENDING→PROCESSING→SUCCEEDED/FAILED）/ resultText? / resultUrl? / cost / error?
- **QuotaLog**：userId / amount / reason（配额变动流水）
- **AuditLog**：userId / action / detail?（注册、改资料、改密码、审批等均落审计）
- **Approval**：requesterId / approverId? / type / amount / reason? / status（PENDING/APPROVED/REJECTED）
...
| GET | /auth/me | 登录 | 当前用户信息（含班级与 OPC 绑定字段，不透出 ssoToken） |
| POST | /auth/refresh | 登录 | 刷新 token |
| PATCH | /auth/profile | 登录 | 修改姓名（个人资料） |
| PATCH | /auth/profile/password | 登录 | 修改密码（需原密码校验） |

### 用户 users（管理）
`GET /users`（分页/筛选）、`GET /users/classes`、`POST /users`、`POST /users/import`（批量）、`PATCH /users/:id`（改角色/资料/重置密码）、`PATCH /users/:id/quota`（配额调整，落 QuotaLog）

### 模型 models
`GET /models`（启用列表）、`GET /models/all`、`POST /models`、`PATCH /models/:id`、`DELETE /models/:id` —— 仅超管可写；模型含服务商 / API Key / Base URL / 模型 ID 等字段

### 作品 works
`POST /works`（提交创作，扣减配额并生成 Work 记录；支持透传 `aspectRatio`（画面比例，如 16:9）/ `resolution`（分辨率 480p/720p/1080p）/ `duration`（视频秒数）参数；参考图支持 `images` 多图数组（图生图逐张以 image[] 字段发送），兼容旧单图 `image` 字段）、`GET /works`（本人作品列表）、`GET /works/stats`（门户首页统计：总次数 + 按类型分布）

### 后台 admin
`GET /admin/stats`、`GET /admin/usage`（本地/云端、按功能分布）、`GET /admin/user-stats?role=`、`GET /admin/audit`、`GET /admin/approvals`、`GET /admin/my-approvals`、`POST /admin/approvals`、`PATCH /admin/approvals/:id`（审批通过自动加配额）、`GET /admin/classes`、`POST /admin/classes`、`GET /admin/quota-users`
...
2. **配额**：创作按所选模型 `cost` 扣点，余额不足则拒绝；仅生成成功后才扣点（失败不扣）；配额调整必须留 QuotaLog 流水。
3. **审批**：单次申请 > 500 点需校领导在「大额审批」处理；APPROVED 后自动到账。
4. **审计**：注册 / 改资料 / 改密码 / 权限变更 / 审批 / SSO 登录等敏感操作写 AuditLog。
5. **AI Provider 与用户 API Key**：`backend/src/ai/providers.ts` 适配层按模型 `provider` 分派 —— OPENAI（文/图/视频，图生图走 multipart `/images/edits`，参考图字段名 `image[]`，豆包系附 `watermark=false`）、DEEPSEEK（文本）、CLAUDE（messages）、DASHSCOPE（文本+文生图）、COMFYUI（文生图工作流 + MiniMax H3 文生视频/图生视频 + Qwen3-TTS 配音）。平台实际只接 genzhi 平台模型，调用密钥优先级：**当前登录用户 SSO 带入的 apikey** > 模型单独配置的 apiKey > `backend/.env` 的 `AI_API_KEY` 全局回退；ComfyUI 模型无需密钥。
6. **画面比例 / 分辨率**：生图类按 `aspectRatio` 计算 size（256 圆整 + 火山系最小 3686400 像素保护）；视频类经 ComfyUI `ResolutionSelector` 映射（比例→枚举标签，分辨率→megapixels：480p=0.3 / 720p=1.0 / 1080p=2.0）。
7. **创作画布**：基于 @vue-flow/core 的无限画布（白色背景 + 点阵网格），右键画布弹出「添加节点 / 工作流模板」菜单（参考 huobao-drama 交互），右键节点弹出「立即生成 / 创建副本 / 删除」菜单；文本 / 生图（文生图↔图生图模式切换）/ 视频生成 / 配音四类节点，按连线链式串联（上游文本自动作下游提示词、上游图片自动作图生图/图生视频输入），每节点独立选模型生成并真实落库，「运行工作流」按序执行全链。
8. **前端静态配置**：业务数据一律走 `src/api`，禁止在 `data/prototype.ts` 添加 mock 业务数据。
...
演示账号（种子数据）：`student / 123456`、`teacher / 123456`、`leader / 123456`、`admin / admin`。
